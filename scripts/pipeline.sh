...
do
    {
        ...
        echo "CUTADAPT: "
        grep -hi -e "Lecturas con adaptadores" log/cutadapt/$sid.log 
        grep -hi -e "Total de pares de bases procesadas" log/cutadapt/$sid.log 
        echo " "
        
        echo "STAR: "
        grep -hi -e " % de lecturas mapeadas de manera única" out/star/$sid/$sid_Log.final.out 
        grep -hi -e "% de lecturas mapeadas a múltiples loci" out/star/$sid/$sid_Log.final.out 
        grep -hi -e "% de lecturas mapeadas a demasiados loci" out/star/$sid/$sid_Log.final.out 
        echo " "
    } >> log/pipeline.log

done

echo "Pipeline log completo"

echo "Pipeline se ha completado con éxito."
